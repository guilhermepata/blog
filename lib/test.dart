...
void main() {
  ...
  Future<String> markdownfile =
      rootBundle.loadString("posts/finding_gender.txt");
  ...
  markdownfile.then((value) {
    ...
    // titleExp = RegExp('===');
    // print(markdown);
    var matches = titleExp.allMatches(value);
    assert(matches.length > 0, 'The article must have a title');
    var title = matches.first.group(1);
    print(title);
  });

  RegExp titleExp = RegExp(r'(.+)\n[=-]{3,}\n*');
  // print(markdown);
